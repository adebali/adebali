...
var sciencescript = function(){return 1;}

loadScript("js/science.v1.js", sciencescript)
...
var TMChart = dc.barChart("#tm-chart");
var LengthChart = dc.barChart("#length-chart");
var TM_pie_chart = dc.pieChart("#TM_pie_chart");
var quarterChart = dc.pieChart("#quarter-chart");
var orthoChart = dc.pieChart("#ortho-chart");
...
//Initialize of Data Formats
	...
	if(d.SeqLen > -1){d.SeqLenOrtho = d.SeqLen;}
	else{d.SeqLenOrtho = SeqLenNAN;}
	d.pident = +d.pident;
	pident_sum += d.pident;
	total_count += 1;
	d.count = 1;
    ...
    var allDimension = ndx.dimension(function(){return 1;});
    var allGroup = allDimension.group();
    var dimensions = [];
    var thegroups = [];

    var myaverage = average(allGroup);
	...
	thegroups[i] = dimensions[i].group();
     ...
     var TM_pie_dimension = ndx.dimension(function (d) {
     return d.TMn == 0 ? "No":"Yes";
     });
     var TM_pie_group = TM_pie_dimension.group();
...
// determine a histogram of percent changes
    ...
    var myTM = ndx.dimension(function(d) {
         return Math.round(d.TMn);
    });
    var myTMGroup = myTM.group();
    

    var nbin = 10; 
    var SeqLenDim = ndx.dimension(function(d) {
         return Math.round(d.SeqLenOrtho/nbin)*nbin;
    });
    var SeqLenGroup = SeqLenDim.group();
    ...
    var orthoDim = ndx.dimension(function (d){
    	return d.SeqLenOrtho == SeqLenNAN ? "Human Specific":"Ortholog";
	});
    var orthoGroup = orthoDim.group();
...
*/
 ...
 /*
	...
	.width(pieWidth)
        .height(pieHeight)
        .radius(pieRadius)
	.dimension(TM_pie_dimension)
	.group(TM_pie_group)
	.label(function (d) {
		if (TM_pie_chart.hasFilter() && !TM_pie_chart.hasFilter(d.key))
		return d.key + "(0%)";
		return d.key + "(" + Math.floor(d.value / all.value() * 100) + "%)";
		});
     orthoChart
        ...
        .group(orthoGroup)
        .label(function (d) {
                if (orthoChart.hasFilter() && !orthoChart.hasFilter(d.key))
                return d.key + "(0%)";
                return d.key + "(" + Math.floor(d.value / all.value() * 100) + "%)";
                });
...
//	.colors(['#3182bd', '#6baed6', '#9ecae1', "red"])
	...
	.xAxisLabel('TM number')
        ...
        //.round(dc.round.floor)
       // .alwaysUseRounding(true)
        .x(d3.scale.linear().domain([-0.5, 15]))
        .renderHorizontalGridLines(true)
        // customize the filter displayed in the control span
        /*.filterPrinter(function (filters) {
            var filter = filters[0], s = "";
	    var rangeEnd =  parseInt(filter[1])-1
            s += parseInt(filter[0]) + "-" + rangeEnd + "";
            return s;
        });

	*/
    LengthChart.width(800)
        .height(140)
        .margins({top: 10, right: 50, bottom: 30, left: 40})
        .dimension(SeqLenDim)
        .group(SeqLenGroup)
	...
	.elasticX(true)
	.yAxisLabel('Pairs')
	.xAxisLabel('Sequence Length (amino acid)')
        // (optional) whether bar should be center to its x value. Not needed for ordinal chart, :default=false
        .centerBar(true)
        // (optional) set gap between bars manually in px, :default=2
        .gap(0)
...
/*
  ...
  */  
    dc.dataTable(".dc-data-table")
        ...
        .columns([
            function (d) {return d.GeneID;},
	    function (d) {return d.pident;},
	    function (d) {return d.SeqLenOrtho;},
    ...
    // var ave = ndx.dimension(function (d) {
     //		s += d.pident;
//		t += 1;
//		}
//		return s/t;
//		);
     ...
     dc.dataTable(".stat-table")
     
     	.dimension(myhistogram)
        // data table does not use crossfilter group but rather a closure
        // as a grouping function
        .group(function(d){return '';})
        .size(1) // (optional) max number of records to be shown, :default = 25
        // dynamic columns creation using an array of closures
        .columns([
	    function (d) {return numberFormat(allDimension.mean(ndx.size(),'pident'));},
	    function (d) {return numberFormat(allDimension.median(ndx.size(),'pident'));}
	    //function (d) {return numberFormat(allDimension.mode(ndx.size(),'pident'));}
        ])
        // (optional) sort using the given field, :default = function(d){return d;}
       .sortBy(function (d) {
            return d.pident;
